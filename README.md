...
. Star Triangle:Uses simple string multiplication ("*" * i). As i grows, the line gets longer.
 
. Reverse Star: Uses a range that counts backwards (range(n, 0, -1)). It starts big and shrinks.

. Number Triangle: Uses a Nested Loop. The inner loop handles the horizontal counting (1, 2, 3...) while the outer loop handles the vertical rows.

. Number Pyramid
 
•	Spaces: We calculate spaces as (Total Size - Current Row). If size is 5 and we are on row 1, we print 4 spaces.
