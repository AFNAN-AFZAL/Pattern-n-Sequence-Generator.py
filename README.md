...
.  Uses a range that counts backwards (range(n, 0, -1)). It starts big and shrinks.

. Number Triangle
 
. : Uses a Nested Loop. The inner loop handles the horizontal counting (1, 2, 3...) while the outer loop handles the vertical rows.
